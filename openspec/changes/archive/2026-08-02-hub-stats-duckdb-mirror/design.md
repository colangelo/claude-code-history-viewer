...
| Disk during a `mirror rebuild` | **~2x the mirror, transiently.** `rebuild` builds a fresh file beside the live one and renames, so it needs ~1.6 GB free at today's 798 MB, not 798 MB. Worth stating because the headroom figure this table used to quote went stale within a day: 121 GiB when measured, **22.5 GB at swap time** (home-network #35) |
