...
- [ ] 5.4 Rebase onto current `main`, merge, bump 0.12.0 + `just sync-version`, release per process (coordinate with the release train — check `git tag --list 'cchv-v*'` for the latest tag first; the train moves fast); deploy relay: §2b swap + one-time model-dir staging
