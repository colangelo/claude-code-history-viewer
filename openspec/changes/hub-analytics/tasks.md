...
- [ ] 8.5 Verify live: statistics load in the deployed webapp and agree with the pre-deploy comparison. **API half DONE 2026-07-25 15:00** — infra swapped the binary (404 → 502 → 200 on `/v1/stats/global` across 14:53–14:55); all three routes correct against live pg1: global 200 in 18.1 s / 20.07B deduped tokens (vs ~40.9B naive, so dedup holds), project 200 in 2.3 s, session 200 in 1.6 s, both not-found paths 404. **Still blocked on two infra steps, relayed on thread `615a4b5a`:** the catch-up backfill has not run (sessions `588159`/`588039`/`588076`/`588038`/`587988` began and ended inside the old-binary gap, are `has_tool_use: true`, and report zero tool rows), and the webapp is still the `v0.13.1` bundle, so the Analytics tab itself is unverified
