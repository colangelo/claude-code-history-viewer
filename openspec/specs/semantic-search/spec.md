...
Embedding-based retrieval over the archive. Today this covers journal
entries: a hub-local CPU embedder, an additive embeddings table with a
self-healing sweep, and the `semantic` / `hybrid` query modes that close the
paraphrase-recall gap plain FTS leaves open. Message-scale embeddings (and the
pgvector path they would need) are a later phase.
