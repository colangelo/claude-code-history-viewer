# timemachine-backfill Specification

## Purpose

Recovering agent history that has aged out of a provider's local retention
(Claude Code prunes after ~30 days) from Time Machine backups into the durable
hub archive. Covers snapshot enumeration, read-only mounting, isolated
fake-home staging that never touches live config or daemon state, one-shot
ingest with per-source-machine identity, foreign backup stores (a retired
machine's disk attached to another Mac), and the operator runbook.
