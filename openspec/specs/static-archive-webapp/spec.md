...
`localStorage` (`cchv.archiveWeb.fontScale`) and re-applied on load. The
webapp's default scale SHALL be 1.1 (the shared type scale is tuned for the
dense desktop viewer; the webapp reads one step larger). Control labels SHALL
be localized with accessible names.
