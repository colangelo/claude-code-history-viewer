//! Pi coding agent (badlogic's `pi` / Pi SDK agent).
//!
...
fn session_meta(data: &str) -> Option<SessionMeta> {
    ...
    for line in data.lines() {
        ...
        // timestamps shown on individual messages — and, since this parser
        // feeds the archive, consistent cross-machine session ordering.
        let ts = msg
            .get("timestamp")
...
fn parse_messages(data: &str) -> Vec<ClaudeMessage> {
    ...
    for line in data.lines() {
        ...
        if let Some(msg) = convert_record(&rec, &session_id) {
            messages.push(msg);
        }
    }
    messages
...
fn convert_record(rec: &Value, session_id: &str) -> Option<ClaudeMessage> {
