...
use crate::models::{ClaudeMessage, ClaudeProject, ClaudeSession, MessagePage, RawLogEntry};
use crate::utils::{
    decode_project_path, detect_git_worktree_info, estimate_message_count_from_size,
    extract_project_name, find_line_ranges, find_line_starts,
};
...
/// Scan a Claude configuration directory (`~/.claude` or a custom dir) and return
/// the projects discovered under its `projects/` subfolder.
pub fn scan_projects(claude_path: &str) -> Result<Vec<ClaudeProject>, String> {
    ...
    for entry in entries {
        ...
        let actual_path = crate::utils::decode_project_path_verified(&project_path)
            ...
            .unwrap_or_else(|| decode_project_path(&project_path));
...
/// Load all Claude sessions for a project directory, using an on-disk metadata
/// cache with incremental parsing for append-only growth.
...
/// Load all messages from a single Claude session JSONL file.
...
pub fn subagents(session_path: &str) -> Result<Vec<SubagentSession>, String> {
...
/// Load a page of messages from a session file (chat-style pagination: offset=0
...
/// Count the valid (viewer-visible) messages in a session file.
#[allow(unsafe_code)] // Required for mmap performance optimization
pub fn message_count(session_path: &str, exclude_sidechain: Option<bool>) -> Result<usize, String> {
    // Use memory-mapped file for faster I/O
    let file =
        fs::File::open(session_path).map_err(|e| format!("Failed to open session file: {e}"))?;
...
mod tests {
    ...
    #[test]
    fn test_load_session_messages_basic() {
        ...
        let result = load_messages(&file_path.to_string_lossy());
    ...
    #[test]
    fn test_load_session_messages_excludes_summary() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_session_messages_empty_file() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_session_messages_with_empty_lines() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_session_messages_file_not_found() {
        let result = load_messages("/nonexistent/path/file.jsonl");
    ...
    #[test]
    fn test_load_session_messages_with_malformed_json() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_session_messages_paginated_basic() {
        let temp_dir = TempDir::new().unwrap();
        ...
        let result = load_messages_paginated(&file_path.to_string_lossy(), 0, 3, None);
    ...
    #[test]
    fn test_load_session_messages_paginated_offset() {
        let temp_dir = TempDir::new().unwrap();
        ...
        // Get second page
        let result = load_messages_paginated(&file_path.to_string_lossy(), 3, 3, None);
    ...
    #[test]
    fn test_load_session_messages_paginated_exclude_sidechain() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
        ...
        let result = load_messages_paginated(&file_path.to_string_lossy(), 0, 10, Some(true));
    ...
    fn test_get_session_message_count() {
        ...
        let result = message_count(&file_path.to_string_lossy(), None);
    ...
    #[test]
    fn test_get_session_message_count_exclude_sidechain() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
        ...
        // Without exclude
        let count_all = message_count(&file_path.to_string_lossy(), None).unwrap();
        assert_eq!(count_all, 3);

        // With exclude
        let count_filtered = message_count(&file_path.to_string_lossy(), Some(true)).unwrap();
        assert_eq!(count_filtered, 2);
    }

    #[test]
    fn test_load_project_sessions_basic() {
        let temp_dir = TempDir::new().unwrap();
        ...
        let result = load_sessions(&temp_dir.path().to_string_lossy(), None);
    ...
    #[test]
    fn test_load_project_sessions_prefers_jsonl_cwd_for_project_name() {
        let temp_dir = TempDir::new().unwrap();
...
"#
        ...
        let result = load_sessions(&project_dir.to_string_lossy(), None).unwrap();
    ...
    #[test]
    fn test_load_project_sessions_prefers_verified_folder_over_stale_cwd() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_project_sessions_with_summary() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_project_sessions_multiple_files() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_load_project_sessions_exclude_sidechain() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
        ...
        // Without exclude
        let result_all = load_sessions(&temp_dir.path().to_string_lossy(), None).unwrap();
        assert_eq!(result_all[0].message_count, 2);

        // With exclude
        let result_filtered =
            load_sessions(&temp_dir.path().to_string_lossy(), Some(true)).unwrap();
        assert_eq!(result_filtered[0].message_count, 1);
    }

    #[test]
    fn test_load_project_sessions_with_tool_use() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_load_project_sessions_empty_directory() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_incremental_parsing_on_file_append() {
        use std::io::Write;
...
"#;
        ...
        let result1 = load_sessions(&temp_dir.path().to_string_lossy(), None).unwrap();
        ...
        let result2 = load_sessions(&temp_dir.path().to_string_lossy(), None).unwrap();
    ...
    #[test]
    fn test_message_with_missing_uuid_generates_new_one() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_message_with_missing_session_id() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_assistant_message_with_usage_stats() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_session_summary_fallback_first_user_message() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
        ...
        let result = load_sessions(&temp_dir.path().to_string_lossy(), None).unwrap();
        assert_eq!(result.len(), 1);
        assert_eq!(
            result[0].summary,
    ...
    #[test]
    fn test_session_summary_fallback_first_assistant_text() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_session_summary_fallback_last_user_message() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_session_summary_fallback_incremental_preserves_values() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_extract_assistant_text_with_string_content() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_extract_assistant_text_min_length() {
        let temp_dir = TempDir::new().unwrap();
...
"#;
    ...
    #[test]
    fn test_should_extract_rename_from_system_message() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_use_last_rename_when_multiple() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_prioritize_rename_over_other_summaries() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_fallback_to_existing_summary() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_not_count_system_as_message() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_ignore_empty_rename() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_phase2_rename_beyond_metadata_lines() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_incremental_append_then_rename() {
        let temp_dir = TempDir::new().unwrap();
        let file_path = temp_dir.path().join("test.jsonl");
    ...
    #[test]
    fn test_should_extract_rename_from_branch_custom_title() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_use_last_naming_event_regardless_of_kind() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_should_ignore_empty_custom_title() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_phase2_custom_title_beyond_metadata_lines() {
        let temp_dir = TempDir::new().unwrap();
    ...
    #[test]
    fn test_incremental_append_then_custom_title() {
        let temp_dir = TempDir::new().unwrap();
        let file_path = temp_dir.path().join("test.jsonl");
    ...
    // ========================================================================
    // scan_projects tests (moved from src-tauri commands/project.rs)
    // ========================================================================

    fn create_project_jsonl_file(dir: &PathBuf, filename: &str, content: &str) {
    ...
    fn test_scan_projects_empty() {
        ...
        let result = scan_projects(&claude_dir.to_string_lossy());
        assert!(result.is_ok());
        assert!(result.unwrap().is_empty());
    }

    #[test]
    fn test_scan_projects_no_projects_dir() {
        let temp_dir = TempDir::new().unwrap();

        let result = scan_projects(&temp_dir.path().to_string_lossy());
        assert!(result.is_ok());
        assert!(result.unwrap().is_empty());
    }

    #[test]
    fn test_scan_projects_single_project() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
    ...
    #[test]
    fn test_scan_projects_multiple_projects() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        ...
        std::fs::create_dir_all(&project1_dir).unwrap();
        create_project_jsonl_file(&project1_dir, "session1.jsonl", "{}");
        create_project_jsonl_file(&project1_dir, "session2.jsonl", "{}");

        // Create project 2
        let project2_dir = projects_dir.join("project-beta");
        std::fs::create_dir_all(&project2_dir).unwrap();
        create_project_jsonl_file(&project2_dir, "session.jsonl", "{}");
    ...
    #[test]
    fn test_scan_projects_extracts_project_name() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        ...
        std::fs::create_dir_all(&project_dir).unwrap();
        create_project_jsonl_file(&project_dir, "session.jsonl", "{}");
    ...
    #[test]
    fn test_scan_projects_prefers_jsonl_cwd_over_lossy_storage_name() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        ...
        let projects = scan_projects(&claude_dir.to_string_lossy()).unwrap();
    ...
    #[test]
    fn test_scan_projects_prefers_verified_folder_over_stale_cwd() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
    ...
    #[test]
    fn test_scan_projects_prefers_top_level_cwd_over_subagent_cwd() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        ...
        std::fs::create_dir_all(&subagent_cwd).unwrap();
    ...
    #[test]
    fn test_scan_projects_sorted_by_last_modified() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        ...
        create_project_jsonl_file(&older_dir, "session.jsonl", "{}");
        ...
        create_project_jsonl_file(&newer_dir, "session.jsonl", "{}");
    ...
    #[test]
    fn test_scan_projects_ignores_non_jsonl_files() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        ...
        create_project_jsonl_file(&project_dir, "session.jsonl", "{}");
        create_project_jsonl_file(&project_dir, "config.json", "{}");
        create_project_jsonl_file(&project_dir, "readme.txt", "readme");
    ...
    #[test]
    fn test_scan_projects_nested_sessions() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
        let project_dir = projects_dir.join("test-project");
        ...
        create_project_jsonl_file(&project_dir, "session1.jsonl", "{}");
        create_project_jsonl_file(&nested_dir, "session2.jsonl", "{}");
    ...
    #[test]
    fn test_scan_projects_skips_empty_project_directories() {
        let temp_dir = TempDir::new().unwrap();
        let claude_dir = temp_dir.path().join(".claude");
        let projects_dir = claude_dir.join("projects");
    ...
    #[cfg(unix)]
    #[test]
    fn test_scan_projects_follows_symlinked_project_dir() {
        ...
        create_project_jsonl_file(&shared_dir, "session.jsonl", "{}");
    ...
    #[cfg(unix)]
    #[test]
    fn test_scan_projects_skips_dangling_symlink() {
        use std::os::unix::fs::symlink;
        ...
        create_project_jsonl_file(&real_dir, "session.jsonl", "{}");
    ...
    #[cfg(unix)]
    #[test]
    fn test_scan_projects_deduplicates_symlink_and_real_dir() {
        use std::os::unix::fs::symlink;
