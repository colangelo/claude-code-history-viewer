//! Antigravity token-state extraction (pure, Tauri-free).
//!
//! Moved from `src-tauri/src/commands/antigravity.rs` during the history-core
//! extraction. The thin Tauri command wrappers remain in src-tauri and call
...
pub fn is_valid_antigravity_session_id(session_id: &str) -> bool {
    !session_id.is_empty()
        && session_id
            .chars()
