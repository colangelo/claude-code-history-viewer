...
//!
//! What does change is where the deduplication happens. Postgres computed
//! `usage_row` per request inside the scope materialization (a window function
//! over the scoped rows, ~3.4 s of a 13.7 s summary). Here it is a stored column
//! the refresher fills once, so materialization is a filter and a join. The one
//! consequence is that `usage_row` is now global rather than window-relative:
//! measured over the live archive, 10 of 2,648,869 dedup groups straddle a date
//! boundary, so a windowed comparison can differ for those and nothing else
//! (design D3).
//!
